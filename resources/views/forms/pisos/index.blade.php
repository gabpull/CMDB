...
@section('content')
    ...
    <div>
        <a href="/pisos/crear" class="btn btn-primary ">
            {{ __('Crear Piso') }}
        </a>
    </div>
    <hr>
    ...
    <table class="table align-middle mb-1 bg-white table-responsive-sm table-hover table-condensed">
        ...
        <tbody>
            @foreach ($pisos as $piso)
                <tr>
                    <td class="align-middle"> {{ $piso->desc_piso }} </td>
                    <td class="align-middle"><span
                            class="badge @if ($piso->activo == 1) badge-success @else badge-warning text-dark @endif rounded-pill">
                            @if ($piso->activo == 1)
                                {{ 'Activo' }}
                            @else
                                {{ 'Inactivo' }}
                    ...
                    <td class="align-middle">
                        <div>
                            <a href="/pisos/{{ $piso->id }}/editar"><svg xmlns="http://www.w3.org/2000/svg"
                            ...
                            <form action="/pisos/{{ $piso->id }}" method="POST" style="display: inline-block">
                                @method('DELETE')
                                @csrf
                                <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Está seguro de eliminar el edificio')">
